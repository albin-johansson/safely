...
/// Performs a bitwise right circular shift of an integer.
///
...
[[nodiscard]] constexpr auto rotr(const T val, const T n) noexcept -> T
{
#if SAFELY_HAS_BUILTIN_ROTATERIGHT
  if constexpr (std::is_same_v<T, u64>) {
    return __builtin_rotateright64(val, n);
  }
  else if constexpr (std::is_same_v<T, u32>) {
    return __builtin_rotateright32(val, n);
  }
  else if constexpr (std::is_same_v<T, u16>) {
    return __builtin_rotateright16(val, n);
  }
